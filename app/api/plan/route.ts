...
function findCol(headers: string[], targets: string[]) {
  ...
  for (const t of tnorm) { const hit = norm.find((x) => x.n === t); if (hit) return hit.raw; }
  for (const t of tnorm) { const hit = norm.find((x) => x.n.includes(t) || t.includes(x.n)); if (hit) return hit.raw; }
  return null;
}
function toInt(v:any){ const n=Number(v); return Number.isFinite(n)?Math.trunc(n):undefined; }
function addDaysISO(iso:string, days:number){ const d=new Date(iso+"T00:00:00Z"); d.setUTCDate(d.getUTCDate()+days); return d.toISOString().slice(0,10); }
function detectFromArabicDescription(desc:string){
  const d=(desc||"").toLowerCase();
  const makeMap: Array<[RegExp,string]> = [
    [/ايسوزو|isuzu/i,"ISUZU"],[/مرسيدس|mercedes|m-?benz/i,"MERCEDES-BENZ"],[/شاكمان|shacman/i,"SHACMAN"],
    [/سينوتراك|sinotruk|howo/i,"SINOTRUK"],[/تويوتا|toyota/i,"TOYOTA"],[/هيونداي|hyundai/i,"HYUNDAI"],
  ];
  let make=""; for(const [rx,val] of makeMap) if(rx.test(d)){make=val;break;}
  const modelMap: Array<[RegExp,string]> = [[/actros/i,"ACTROS"],[/npr/i,"NPR"],[/x3000/i,"X3000"],[/howo/i,"HOWO"],[/camry/i,"CAMRY"],[/staria/i,"STARIA"]];
  let model=""; for(const [rx,val] of modelMap) if(rx.test(d)){model=val;break;}
  const heavy=/قلاب|شاحنة|دينا|تريلا|تريله|رأس|راس|قاطرة|سكس|معدات|حفار|ونش|باص|اتوبيس/i.test(d);
  return { make, model, category: heavy ? "HEAVY" : "" };
}
...
export async function POST(req:Request){
  ...
  for(const r of json){
    ...
    if((!make||!model||!category)&&desc){
      ...
      make=make||d.make; model=model||d.model; category=category||d.category;
    }
  ...
  for(const r of outRows){
    ...
    const o=byGroup.get(g); o.count+=1; o.devices[dev]=(o.devices[dev]||0)+1;
  }
  ...
  if(opt.includeRecommendations !== false){
    ...
    outRows.forEach((r)=>{ const rr={...r}; delete rr["__group"]; sheet.addRow(rr); });
  }
  ...
  if(opt.includePdf !== false){
    ...
    page.drawText(lang==="ar"?"ملخص:":"Summary:",{x:40,y:775,size:11,font:bold});
    const counts = outRows.reduce((acc:any,r:any)=>{ const d=r["Recommended Teltonika Device"]; acc[d]=(acc[d]||0)+1; return acc; },{});
    let y=758;
    for(const [k,v] of Object.entries(counts)){ if(y<80) break; page.drawText(`${k}: ${v}`,{x:40,y,size:10,font}); y-=14; }
    if(includePlan){
      page.drawText(lang==="ar"?"خطة التركيب (Top 10):":"Installation plan (Top 10):",{x:40,y:220,size:11,font:bold});
      let yy=202;
      for(const r of planRows.slice(0,10)){ if(yy<70) break; const line=`${r["Region/Group"]} | Vehicles:${r["Vehicles"]} | Days:${r["Est. Days"]} | ${r["Start"]}→${r["End"]}`; page.drawText(line.slice(0,120),{x:40,y:yy,size:9,font}); yy-=12; }
    ...
    if(includeCost){
      ...
      const lines=[`Fuel/day: ${fuelCostPerDay.toFixed(2)}`,`Tech/day: ${techCostPerDay.toFixed(2)}`,`PerDiem/day: ${perDiemCostPerDay.toFixed(2)}`,`Hotel/day: ${hotelCostPerDay.toFixed(2)}`,`TOTAL/day: ${totalCostPerDay.toFixed(2)}`,`TOTAL: ${totalCost.toFixed(2)}`];
      let yyy=240; for(const ln of lines){ page.drawText(ln,{x:320,y:yyy,size:10,font}); yyy-=14; }
    }
